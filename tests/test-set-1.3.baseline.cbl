      **> clean-cobol.py --clear-left-margin
       IDENTIFICATION DIVISION.                                         AST
       PROGRAM-ID. TEST-SET-1.                                          AST
                                                                        AST
       ENVIRONMENT DIVISION.                                            AST
                                                                        AST
       INPUT-OUTPUT SECTION.                                            AST
                                                                        AST
       FILE-CONTROL.                                                    AST
                                                                        AST
       SELECT INPUT-FILE ASSIGN TO 'input-file.text'                    AST
           ORGANIZATION IS LINE SEQUENTIAL.                             AST
                                                                        AST
       DATA DIVISION.                                                   AST
                                                                        AST
       FILE SECTION.                                                    AST
       FD INPUT-FILE.                                                   AST
       01 INPUT-RECORD PIC X(80).                                       AST
                                                                        AST
                                                                        AST
       WORKING-STORAGE SECTION.                                         AST
002300*> USED BY PROCESS-COMMAND-ARGS.CBL AS WELL AS APPS THAT          AST
       *> CALL PROCESS-COMMAND-ARGS.                                    AST
                                                                        AST
       copy "param-group.cpy".                                          AST
                                                                        AST
                                                                        AST
                                                                        AST
       01 WORK-VARS.                                                    AST
           05 flag-1 pic x value 'N'.                                   AST
               88 flag-1-valid value 'Y'.                               AST
           05 flag-2 pic x value 'Y'.                                   AST
               88 flag-2-valid value 'Y'.                               AST
           05 flag-3 pic x value 'N'.                                   AST
           05 flag-4 pic x value 'Y'.                                   AST
           05 flag-5 pic x value 'N'.                                   AST
           05 flag-6 pic x value 'Y'.                                   AST
           05 change-percent pic zz9.99- value space.                   AST
                                                                        DECAST
           05 idx-1 pic 9(3).                                           AST
           05 msg pic x(80).                                            AST
           05 eof-switch pic x value 'N'.                               AST
               88 input-file-eof value 'Y' false 'N'.                   AST
                                                                        AST
                                                                        AST
                                                                        AST
       PROCEDURE DIVISION.                                              AST
                                                                        AST
       MAIN.                                                            AST
           call 'process-command-args' using PARAM-GROUP                AST
               ON EXCEPTION                                             AST
                   DISPLAY 'the SUBROUTINE CALL TO process-command-args AST
      -                ' FAILED!'                                       AST
           end-call
                                                                        AST
           perform do-stuff                                             AST
           perform do-stuff-2                                           AST
           perform do-stuff-3                                           AST
           perform do-stuff-4                                           AST
           perform do-stuff-5                                           AST
           perform do-stuff-6                                           AST
           perform do-stuff-7                                           AST
           perform do-stuff-8                                           AST
           perform do-stuff-9                                           AST
                                                                        AST
           perform do-stuff-10                                          AST
                                                                        AST
                                                                        DECAST
                                                                        DECAST
       GOBACK                                                           DECAST
       ...
       *> test handling of 'if', 'else', 'then'                         AST
       DO-STUFF.                                                        DECAST
           ...
           *> t1: test if statement by itself                           DECAST
           if flag-1 = 'N'                                              AST
               display 't1: flag-1 is false'                            AST
           end-if *> flag-1 = 'N'
                                                                        AST
           *> t2: test if with a two part expression                    AST
           if flag-1 = 'Y' or flag-2 = 'Y'                              AST
               display 't2: at least one of two flags is ''Y'''         AST
